...
export default function AiChatSection() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "assistant",
      content: `Hello! I am Irfan's AI Assistant, powered by AWS Bedrock architecture. Ask me anything about Irfan's background, cloud deployments, microservices, or enterprise experience!`,
    },
  ]);
    ...
    <section id="ai-assistant" className="max-w-4xl mx-auto px-6 md:px-12 py-24 relative z-10">
      ...
      <div className="card-cyber-static rounded-2xl overflow-hidden border border-navy-600 shadow-2xl flex flex-col h-[620px]">
        ...
        <div className="px-6 py-4 bg-navy-900 border-b border-navy-600/80 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[11px] font-mono px-2.5 py-0.5 rounded bg-navy-800 text-cyber-teal border border-cyber-teal/30 flex items-center gap-1.5">
          ...
          {messages.map((msg, idx) => {
            ...
            return (
              ...
              >
                {/* Avatar */}
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-mono font-bold shadow-md ${isUser
                ...
                {/* Message Bubble */}
                <div
                  className={`p-4 rounded-2xl text-xs sm:text-sm leading-relaxed ${isUser
        ...
        <div className="px-4 py-2.5 bg-navy-950/90 border-t border-navy-800/80 text-center">
          <p className="text-[11px] sm:text-xs font-mono text-amber-300/80 flex items-center justify-center gap-1.5 leading-tight">
            <AlertCircle className="w-3.5 h-3.5 text-amber-400 flex-shrink-0" />
            <span>
              <strong>Warning:</strong> AI responses may not be correct and do not represent Irfan&apos;s official views. Please verify critical details directly.
            </span>
          </p>
        </div>
      </div>
